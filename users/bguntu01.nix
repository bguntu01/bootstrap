...
{ username, ... }:
{
  ...
  home-manager.users.${username} = { pkgs, ... }: {
    ...
    programs.git = {
      settings.user.name = "Balaji Guntur";
      settings.user.email = "balaji.guntur@hoptek.ai";
      signing = {
        # Your Bitwarden SSH PUBLIC key (starts with "ssh-ed25519 ..."). Used to sign commits.
        key = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEI9IQjdG2R8Ju4ZdSmeZ/602QHnSfXchlle8bMPdMji default";
        signByDefault = true;
      };
    };
